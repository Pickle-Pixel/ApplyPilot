...
def generate_dashboard(output_path: str | None = None) -> str:
    ...
    def _site_color(site: str) -> str:
        if site in _KNOWN_COLORS:
            return _KNOWN_COLORS[site]
        h = hash(site) % 360
        return f"hsl({h}, 60%, 45%)"

    colors = {s: _site_color(s) for s in set(j["site"] or "" for j in jobs)}
    colors.update(_KNOWN_COLORS)
    ...
    for j in jobs:
        ...
        job_sections += f"""
        <div class="job-card" data-score="{score}" data-site="{escape(j["site"] or "")}" data-location="{escape(location.lower())}">
